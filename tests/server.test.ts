...
describe("Wildcard routes", () => {
  it.each(["/1", "/tasksss", "/taz", "whatever/here"])(
    "GET %s — returns 404 with wildcard error shape",
    async (url) => {
...
describe("Error handling", () => {
  ...
  it("should handle TaskNotFound error and return 404", async () => {
  ...
  it("should handle ZodError and return 400 with issues", async () => {
    const res = await testApp.request("/tasks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
